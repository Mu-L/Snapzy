...
- The live overlay is a `.nonactivatingPanel` and never becomes key (activating would dim live windows), so AppKit's cursor-rect machinery does not self-heal the crosshair after an external reset. The crosshair is kept sticky during a manual drag by re-asserting it from the existing selection drag monitors (`AreaSelectionController.reassertManualSelectionCursor`) plus an `NSApplication.didBecomeActiveNotification` recovery, both torn down with the drag monitors.
